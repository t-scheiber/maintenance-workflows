...
import {nativeDatabase,sameDatabase} from './runtime-database.mjs';
...
export function checked(execute,args,env,extra={},diagnostics){
 const r=execute('docker',args,{env,timeout:180000,maxBuffer:2_000_000,...extra});
 if(diagnostics)for(const key of ['stdout','stderr'])fs.writeFileSync(diagnostics+'.'+key,Buffer.from(r[key]??'').subarray(0,2_000_000),{flag:'wx',mode:0o600});
 if(r.error||r.status!==0)throw Error('Bounded runtime container operation failed');return r.stdout;
}
export function runtimeScanArguments({containerName,scanner,output,archive,cache}){
 return ['run','--name',containerName,'--platform','linux/amd64','--network','none','--read-only','--user',`${process.getuid()}:${process.getgid()}`,'--cap-drop','ALL','--security-opt','no-new-privileges','--pids-limit','128','--memory','4g','--cpus','2','--tmpfs','/tmp:rw,nosuid,nodev,size=2g','--env','HOME=/tmp','--mount',`type=bind,src=${scanner},dst=/scanner,readonly`,'--mount',`type=bind,src=${output},dst=/out`,'--mount',`type=bind,src=${archive},dst=/runtime.tar.gz,readonly`,'--mount',`type=bind,src=${cache},dst=/cache,readonly`,'--entrypoint','/scanner',runtimePolicy('node22').baseImage,'image','--input','/runtime.tar.gz','--config','/out/trivy.yaml','--ignorefile','/out/ignore','--cache-dir','/cache','--skip-db-update','--offline-scan','--no-progress','--cache-backend','memory','--scanners','vuln','--list-all-pkgs','--severity','UNKNOWN,LOW,MEDIUM,HIGH,CRITICAL','--exit-code','0','--format','json','--output','/out/report.json'];
}
export function runRuntimeScan({execute,args,childEnv,directory,output,operations,containerName}){
 const startedAt=new Date().toISOString();let scan,cleanup;
  try{scan=execute('docker',args,{env:childEnv,timeout:600000,maxBuffer:2_000_000});}
  finally{
   // Stop the container before safely capturing its bounded report and diagnostics.
   try{checked(execute,['rm','-f',containerName],childEnv,{timeout:30000},path.join(operations,'cleanup'));cleanup=true;}catch{cleanup=false;}
   if(scan)for(const key of ['stdout','stderr'])fs.writeFileSync(path.join(directory,key+'.log'),Buffer.from(scan[key]??'').subarray(0,2_000_000),{flag:'wx',mode:0o600});
   const reportFile=path.join(output,'report.json');
   if(cleanup&&fs.existsSync(reportFile))fs.writeFileSync(path.join(directory,'report.json'),read(reportFile,32_000_000),{flag:'wx',mode:0o600});
  }
...
export async function prepareRuntime(kind,{env=process.env,execute=spawnSync,save=saveRuntimeImage,nativeAdmission=admitSourceScanners,identify=preparationIdentity}={}){
 ...
 const operations=path.join(directory,'operations');fs.mkdirSync(operations,{mode:0o700});
 ...
 try{
  let tag;
  if(kind==='node22'){tag=policy.baseImage;checked(execute,['pull','--platform','linux/amd64',tag],childEnv,{timeout:600000},path.join(operations,'pull'));}
  else{
   ...
   checked(execute,['build','--platform','linux/amd64','--build-arg','TARGETARCH=amd64','--tag',tag,'-'],childEnv,{input:recipe,timeout:900000},path.join(operations,'build'));
  }
  const inspection=JSON.parse(checked(execute,['image','inspect',tag],childEnv,{},path.join(operations,'inspect')));if(!Array.isArray(inspection)||inspection.length!==1||inspection[0].Os!=='linux'||inspection[0].Architecture!=='amd64')throw Error('Runtime build platform differs');receipt.executionImage=immutable(inspection[0].Id);
  ...
  const database=nativeDatabase(path.join(temporary,'native-bundle-verification'),runIdentity);
  const args=runtimeScanArguments({containerName,scanner,output,archive,cache:database.cache});
  const {scan,startedAt,finishedAt}=runRuntimeScan({execute,args,childEnv,directory,output,operations,containerName});
  sameDatabase(database,nativeDatabase(path.join(temporary,'native-bundle-verification'),runIdentity));
  if(scan.error||scan.status!==0)throw Error('Complete runtime scanner failed');
